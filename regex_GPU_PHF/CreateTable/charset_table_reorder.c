...
DFA_node *NFA2DFA(NFA_node *NFA_init_state, int *state_num, int *final_state_num);
int mark_DFA_id(DFA_node *init_state, int final_state_num);
...
DFA_node *NFA2DFA(NFA_node *NFA_init_state, int *state_num, int *final_state_num) {
    ...
    *state_num = 1 + 1;   // useless state 0 + initial state
    *final_state_num = 0;
    ...
    while (queue_head != queue_tail) {
        ...
        for (ch = 0; ch < CHAR_SET; ch++) {
            ...
            if (num_active_state > 0) {
                qsort(active_states, num_active_state, sizeof(int), comp_int);
                ...
                if (next_DFA_node != NULL) {
                    ...
                    *final_state_num += is_final_state;
...
int mark_DFA_id(DFA_node *init_state, int final_state_num) {
    ...
    int queue_head, queue_tail;
    DFA_node *cur_state;
    int state_count = final_state_num + 1;
    int final_state_count = 1;
    ...
    while (queue_head != queue_tail) {
        ...
        if (cur_state->DFA_state_id == -1) {
            if (cur_state->num_output > 0)
                cur_state->DFA_state_id = final_state_count++;
            else
                cur_state->DFA_state_id = state_count++;
        }
        else {
            continue;
        }
...
int DFA_BFS(DFA_node *init_state) {
    ...
    while (queue_head != queue_tail) {
        ...
        if (cur_state->num_output > 0) {
            // showDFS: fprintf(stdout, "state=%2d  output_pattern=", cur_state->DFA_state_id);
            // showDFS: for (i = 0; i < (cur_state->num_output); i++ )
                // showDFS: fprintf(stdout, "%2d ", cur_state->output_pattern[i]);
            // showDFS: fprintf(stdout, "\n");
            num_output[cur_state->DFA_state_id] = cur_state->num_output;
            outputs[cur_state->DFA_state_id] = cur_state->output_pattern;
        }

        for (ch = 0; ch < CHAR_SET; ch++) {
            if ( cur_state->next_state[ch] != NULL ) {
                sid = (cur_state->next_state[ch])->DFA_state_id;
                // showDFS: fprintf(stdout, "state=%2d  %c ->  %2d\n", cur_state->DFA_state_id, ch, sid);
                queue[queue_tail] = cur_state->next_state[ch] ;
                queue_tail = (queue_tail + 1) % 4000;
